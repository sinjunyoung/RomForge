...
**WUA 파일 하나에 본편 + 업데이트 + DLC 여러 개가 합쳐져 있을 수 있습니다** ([Switch - 병합](switch-merge.md)과 비슷하게, Wii U에서도 여러 콘텐츠를 하나의 WUA로 묶어둘 수 있기 때문입니다). 이런 WUA를 변환하면, **안에 들어있는 타이틀 각각에 대해 결과물이 따로따로 생성됩니다.** 예를 들어 본편+DLC 2개가 든 WUA를 WUP로 변환하면, WUP 폴더 3개가 나옵니다. 반대로 순수 WUD/WUX(디스크 이미지)는 보통 타이틀이 하나뿐이라 결과물도 하나만 나옵니다.
