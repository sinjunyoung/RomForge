...
2. **아이콘**
3. **로고**
4. **배경 이미지** — 2~4번 이미지 영역 아래에는 각각 **비우기** 버튼이 있어, 자동으로 받아온 이미지를 지우고 빈 상태로 되돌릴 수 있습니다.
5. **POPS 에뮬레이터 설정** — **동영상 프리징 픽스 (cmd 0x00)**, **추가 옵션 (cmd 0x0B)** 두 체크박스가 있습니다. 동영상 재생 중 프리징이 발생하면 두 옵션을 모두 체크한 뒤 변환하세요. **단, 첫 번째로 추가한 디스크가 이미 알려진 게임(내장 데이터베이스에 프리셋이 있는 타이틀)이면, "프리셋 설정 로드됨 (수동 설정 비활성화)"라는 문구와 함께 이 설정 영역이 자동으로 잠겨서 직접 수정할 수 없습니다.** 이미 검증된 값이 자동 적용되기 때문입니다.
6. **진행도**
7. **파일 추가**
8. **폴더 추가**
9. **선택 삭제**
...
14. **로그**
15. **부트로고**
16. **비우기** — 지정한 부트로고를 지우고 빈 상태로 되돌립니다. 비워두면 부트로고 없이 출력됩니다.
