// WuaReader.cs
//
// C# port of Exzap's ZArchive reader (https://github.com/Exzap/ZArchive, MIT license,
// src/zarchivereader.cpp) — the counterpart to WuaWriter.cs. A .wua file is a .zar
// (ZArchive) file with a naming convention on top, so this reads .wua files directly.
//
// Dependency: ZstdSharp.Port (NuGet) for decompression — same package WuaWriter.cs uses.
//
// Note: this port skips the original's LRU block cache (it exists purely for read
// performance under repeated random access; correctness doesn't depend on it) and reads
// each requested block fresh. Add caching later if extraction performance matters.
...
public sealed class WuaDirEntry
{
    ...
    public string Name = "";
    public ulong Size; // 0 for directories
}
...
public sealed class WuaReader : IDisposable
{
    ...
    private byte[] _nameTable = Array.Empty<byte>();
    private readonly List<FileDirectoryEntry> _fileTree = new();
    ...
    public static WuaReader OpenFromStream(Stream stream)
    {
        ...
        {
            sections[i] = (
                BinaryPrimitives.ReadUInt64BigEndian(footer.AsSpan(i * 16, 8)),
                BinaryPrimitives.ReadUInt64BigEndian(footer.AsSpan(i * 16 + 8, 8)));
        }
        ...
        if (magic != FooterMagic) throw new InvalidDataException("Not a .wua/.zar file (magic mismatch).");
        if (version != FooterVersion1) throw new InvalidDataException("Unsupported .wua/.zar version.");
        if ((long)totalSize != fileSize) throw new InvalidDataException("Footer totalSize does not match the actual file size.");
        ...
        if (recSize % 40 != 0) throw new InvalidDataException("Offset record section size is invalid.");
        int recordCount = (int)(recSize / 40);
        stream.Position = (long)recOffset;
        for (int i = 0; i < recordCount; i++)
        {
            var buf = new byte[40];
            if (stream.Read(buf, 0, 40) != 40) throw new InvalidDataException("Could not read offset records.");
        ...
        if (nameSize > 0x7FFFFFFF) throw new InvalidDataException("Name table too large.");
        ...
        if (treeSize % FileDirectoryEntrySize != 0) throw new InvalidDataException("File tree section size is invalid.");
        int nodeCount = (int)(treeSize / FileDirectoryEntrySize);
        if (nodeCount == 0) throw new InvalidDataException("File tree is empty.");
    ...
    /// <summary>Looks up a '/'-separated path starting from the root. Returns InvalidNodeHandle if not found.</summary>
    public uint LookUp(string path)
    {
        ...
        foreach (var part in path.Split('/', StringSplitOptions.RemoveEmptyEntries))
        {
            ...
            if (entry.IsFile) return InvalidNode;
            ...
            if (match == InvalidNode) return InvalidNode;
            currentNode = match;
        }
        return currentNode;
    }
    ...
    public ulong GetFileSize(uint node)
        => node < _fileTree.Count && _fileTree[(int)node].IsFile ? _fileTree[(int)node].FileSize : 0;
    ...
    public bool GetDirEntry(uint node, uint index, out WuaDirEntry dirEntry)
    {
        ...
        if (node >= _fileTree.Count) return false;
        var dir = _fileTree[(int)node];
        if (dir.IsFile || index >= dir.Count) return false;
    ...
    /// <summary>Enumerates every file under the whole archive as (fullPath, node).</summary>
    public IEnumerable<(string Path, uint Node)> EnumerateFiles() => EnumerateFiles(RootNode, "");
    ...
    {
        if (node >= _fileTree.Count) return 0;
    ...
    // returns "" on failure, matching the original's empty-string-on-failure behavior
    private static string GetName(byte[] nameTable, uint nameOffset)
    {
        if (nameOffset == 0x7FFFFFFF || nameOffset >= nameTable.Length) return "";
        int pos = (int)nameOffset;
        int nameLength = nameTable[pos] & 0x7F;
        if ((nameTable[pos] & 0x80) != 0)
        {
            if (pos + 1 >= nameTable.Length) return "";
            // NOTE: this mirrors an apparent bug in the original ZArchive reader, which reads
            // the extended length byte from the SAME position instead of position+1. Preserved
            // here for byte-for-byte compatibility; only affects names >= 128 bytes long, which
            // basically never occurs in practice for Wii U title paths.
        ...
        if (pos + nameLength > nameTable.Length) return "";
