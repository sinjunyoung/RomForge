...
namespace NUSPacker.Nuspackage
{
    ...
    public class Ticket
    {
        ...
        public byte[] GetAsData()
        {
            ...
            buffer.Put(Utils.Utils.HexStringToByteArray("00010004"));
            byte[] randomData = new byte[0x100];
            rng.NextBytes(randomData);
            buffer.Put(randomData);
            buffer.Put(new byte[0x3C]);
            buffer.Put(Utils.Utils.HexStringToByteArray("526F6F742D434130303030303030332D58533030303030303063000000000000"));
            buffer.Put(new byte[0x5C]);
            buffer.Put(Utils.Utils.HexStringToByteArray("010000"));
            buffer.Put(GetEncryptedKey().GetKey());
            buffer.Put(Utils.Utils.HexStringToByteArray("000005"));
            randomData = new byte[0x06];
            rng.NextBytes(randomData);
            buffer.Put(randomData);
            buffer.Put(new byte[0x04]);
            buffer.PutLong(GetTitleID());
            buffer.Put(Utils.Utils.HexStringToByteArray("00000011000000000000000000000005"));
            buffer.Put(new byte[0xB0]);
            buffer.Put(Utils.Utils.HexStringToByteArray("00010014000000AC000000140001001400000000000000280000000100000084000000840003000000000000FFFFFF01"));
