...
namespace NUSPacker.Nuspackage.Crypto
{
    public class Encryption
    {
        ...
        public void EncryptFileWithPadding(NUSPacker.Nuspackage.FST fst, string output_filename, short contentID, int BLOCKSIZE)
        {
            ...
            IV ivObj = new IV(JByteBuffer.Allocate(0x10).PutShort(contentID).Array());
        ...
        public void EncryptSingleFile(Stream in_, Stream out_, long length, IV? iv, int BLOCKSIZE, Action<long>? onBytesEncrypted)
        {
            ...
            do
            {
                ...
                if (cur_position + BLOCKSIZE > inputSize)
                {
                    ...
                    inBlockBufferRead = Utils.Utils.GetChunkFromStream(in_, blockBuffer, overflow, expectedSize);
        ...
        private void EncryptFileHashed(Stream in_, Stream out_, long length, Content content, ContentHashes hashes, Action<long>? onBytesEncrypted)
        {
            ...
            do
            {
                ...
                if (output.Length != BLOCKSIZE) Console.WriteLine("WTF?");
                ...
                if ((block % 100) == 0)
                {
                    Console.Write("\rEncryption: " + progress + "%");
                }
        ...
        {
            IV ivObj = new IV(JByteBuffer.Allocate(16).PutShort((short)content_id).Array());
