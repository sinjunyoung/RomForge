...
{
    public static ITitleSource Open(string inputPath, string? keysTxtPath = null) => OpenAll(inputPath, keysTxtPath)[0];
    ...
    public static IReadOnlyList<ITitleSource> OpenAll(string inputPath, string? keysTxtPath = null)
    {
        ...
        switch (ext)
        {
            case ".wua":
                {
                    var reader = WuaReader.OpenFromFile(inputPath);
                    var titles = WuaTitleSource.OpenAllTitles(reader);
                    return [.. titles];
                }
            ...
            case ".wux":
                {
                    ...
                    try
                    {
                        byte[]? discKey = WuDiscReader.FindDiscKey(wud, inputPath, keys.KeyCandidates) ?? throw new InvalidDataException(
                                "Could not find a working disc key. None of the candidates in keys.txt matched, " + "and no companion \"<name>.key\" file was found next to the input.");
